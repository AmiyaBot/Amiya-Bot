...
def get_data():
    ...
    if time.time() - get_data_time > time_threshold:
        covid_data = json.loads(
            json.loads(requests.get("https://view.inews.qq.com/g2/getOnsInfo?name=disease_h5").text)["data"]
        )
        vaccine_top_data = json.loads(requests.get("https://api.inews.qq.com/newsqa/v1/automation/modules/list?"
                                                   "modules=VaccineTopData").text)["data"]
...
def city_covid(prov_name: str, city_name: str):
    try:
        get_data()
...
async def _(data: Message):
    ...
    if len(data_list) == 1:
        ...
        result = prov_covid(data_list[1])
    else:
        result = city_covid(data_list[1], data_list[2])
    place = ""
