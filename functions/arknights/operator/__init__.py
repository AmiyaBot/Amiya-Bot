...
async def _(data: Message):
    ...
    if '技能' in data.text:
        result = await OperatorData.get_skills_detail(info)
        if result:
            return reply.html('template/operator/skillsDetail.html', result)
    else:
        result, tokens = await OperatorData.get_operator_detail(info)
        if result:
            if '召唤物' not in data.text:
                reply = reply.html('template/operator/operatorInfo.html', result)
            elif not tokens['tokens']:
                return Chain(data).text('博士，干员%s未拥有召唤物' % result['info']['name'])
            if tokens['tokens']:
                reply = reply.html('template/operator/operatorToken.html', tokens)
            return reply
