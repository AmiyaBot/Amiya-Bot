...
class GroupEvents:
    ...
    class BotInvitedJoinGroupRequestEvent(Event):
        ...
        async def handle(self, allow: bool = False, message: str = ''):
            await self.ws.http_client.post('/resp/botInvitedJoinGroupRequestEvent', {
                'sessionKey': self.ws.http_client.session,
...
def mirai_message_formatter(account: int,
                            data: dict,
                            websocket: WSClientDefinition = None) -> Union[Message, Event, None]:
    """
    Mirai 消息解析
    ...
    if data['type'] == 'FriendMessage':
        ...
        if hasattr(Mirai, data['type']):
            msg = getattr(Mirai, data['type'])(data, websocket)
        else:
            msg = Event(data['type'], data, websocket)
        return msg
