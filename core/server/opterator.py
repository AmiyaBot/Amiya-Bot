...
class Operator:
    ...
    async def get_all_operator(self):
        return app.response(query_to_list(OperatorIndex.select()))

    @app.route()
    async def get_operator(self, data: QueryData):
        select = OperatorIndex.select(OperatorIndex, OperatorConfig).join(
            OperatorConfig,
            'left join',
            on=(OperatorConfig.operator_name == OperatorIndex.name),
        )

        if data.search:
            select = select.where(
                OperatorIndex.name.contains(data.search) | OperatorIndex.en_name.contains(data.search)
            )
    ...
    async def set_operator(self, data: OperatorConfigModel):
        if OperatorConfig.get_or_none(operator_name=data.name):
            OperatorConfig.update(operator_type=data.operator_type).where(
                OperatorConfig.operator_name == data.name
            ).execute()
        else:
